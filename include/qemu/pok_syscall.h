...
typedef struct
{
   ...
   bool_t                           full;
   pok_port_size_t                  size;
   pok_port_size_t                  off_b; /* Offset of the beginning
                                            * of the buffer */
   pok_port_size_t                  off_e; /* Offset of the end of
                                            * the buffer */
   pok_port_direction_t             direction;
...
typedef enum
{
   POK_PARTITION_MODE_INIT_COLD = 1, /**< Partition is in cold mode
                                        (init mode), only the main
                                        thread is active */
   POK_PARTITION_MODE_INIT_WARM = 2, /**< Partition is in warm init
                                        mode, only the main thread is
                                        active */
   POK_PARTITION_MODE_NORMAL    = 3, /**< Partition is currently being
                                        executed, main thread is
                                        disabled, other threads are
                                        running
                                     */
   POK_PARTITION_MODE_IDLE      = 4, /**< Idle mode, the partition is
                                      * not executed */
   POK_PARTITION_MODE_RESTART   = 5, /**< Currently being restarted */
   POK_PARTITION_MODE_STOPPED   = 6, /**< Partition is stopped, it
